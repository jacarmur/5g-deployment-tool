...
NUMBER_OF_RECEIVERS = 15000;
BW = 100; % MHz
IS_COVERAGE_MODE = false;
MAX_NUMBER_OF_ATTEMPTS = 2;
UMA_TX_POWER = 40; % Watts = 44 dBm
UMI_COVERAGE_TX_POWER = 5;
UMI_HOTSPOT_TX_POWER = 5;
UMI_BLIND_SPOT_TX_POWER = 5;
UMI_ISD = 300; % metters
UMA_FREQUENCY = 3.5e9;
UMI_COVERAGE_FREQUENCY = 6e9;
UMI_HOTSPOT_FREQUENCY = 18e9;
UMI_BLIND_SPOT_FREQUENCY = 12e9;
lat_min = 37.1477;
lon_min = -3.6097;
lat_max = 37.1647;
lon_max = -3.5875;
...
if IS_COVERAGE_MODE
    ...
    while ~best_sinr_data_reached && attempts < MAX_NUMBER_OF_ATTEMPTS
        ...
        [merged_latitudes, merged_longitudes, merged_grid_size, best_sinr_data] = calculate_sinr_values_map([umi_transmitters uma_transmitters], coordinates_bbox);
        best_sinr_data_reached = ~ismember(1, best_sinr_data < 0);
        [umi_data_latitudes, umi_data_longitudes, best_umi_grid_size, umi_sinr_data] = calculate_sinr_values_map(umi_transmitters, coordinates_bbox);
        attempts = attempts + 1;
    end
else
    ...
    [merged_latitudes, merged_longitudes, merged_grid_size, best_sinr_data] = calculate_sinr_values_map([uma_transmitters umi_transmitters], coordinates_bbox);
...
umi_map = siteviewer('Buildings', 'map.osm');
...
%% Summary
coverage = count/NUMBER_OF_RECEIVERS*100;
users_coverage = NUMBER_OF_RECEIVERS - count;
traffic = sum(total_capacity)/NUMBER_OF_RECEIVERS;

coverage_message = " - Coverage: " + coverage + " % (area with SINR < 0 dB)"; 
users_coverage_message = " - Users whose SINR < 0 dB: " + users_coverage;
...
fprintf(file_id, '\n------- SUMMARY ------- \n');
fprintf(file_id, '%s\n%s\n%s\n%s\n%s\n%s\n', coverage_message, users_coverage_message, total_network_traffic, mean_traffic, total_uma, total_umi);
fclose(file_id);
end